...
function equip(item){
 if(item.includes('剑')){
  player.atk-=player.weapon?10:0;
  player.weapon=item;
  player.atk+=10;
  weaponSlot.innerText=item;
 }
 if(item.includes('甲')){
  player.def-=player.armor?5:0;
  player.hp-=player.armor?20:0;
  player.armor=item;
  player.def+=5;
  player.hp+=20;
...
function updateCharacter(){
 let c='🙂';
 if(player.weapon)c='🗡️'+c;
 if(player.armor)c='🛡️'+c;
...
function goTown(){
 ...
 <button onclick="buy('骑士甲',150)">骑士甲</button>`;
}

function goForest(){fight(['哥布林','史莱姆','猫妖','狐妖'][Math.floor(Math.random()*4)]);}
function goLava(){fight(['骷髅','僵尸','蝙蝠妖','黑龙'][Math.floor(Math.random()*4)]);}
